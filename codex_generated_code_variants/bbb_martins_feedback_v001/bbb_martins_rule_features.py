...
def _compute_pka_summary(smiles: str, mol) -> dict[str, float]:
    ...
    result = {
        # Keep the pKa-related columns numeric even when MolGpKa does not report
        # a particular site type, so the train-time preprocessor does not drop the
        # whole block as NaN-only features.
        "most_basic_pka": 0.0,
