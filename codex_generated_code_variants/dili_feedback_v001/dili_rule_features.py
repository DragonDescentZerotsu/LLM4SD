...
def _feature_specs() -> list[tuple[str, str]]:
    return [
        ...
        ("aromatic_ring_density", "Aromatic ring count normalized by total ring count."),
        ...
        ("structural_alert_count", "Number of high-confidence DILI structural alert families matched."),
        ("reactive_metabolite_alert_count", "Number of distinct reactive-metabolite-oriented alert families matched."),
        ("heteroatom_fraction", "Heteroatom count normalized by heavy atom count."),
...
def featurize_smiles(smiles: str) -> dict[str, float]:
    ...
    structural_alert_count = float(sum(reactive_alert_present_flags.values()))
    ...
    aromatic_ring_density = aromatic_ring_count / ring_count if ring_count > 0 else 0.0
    heteroatom_count = _as_float(rdkit.rdMolDescriptors.CalcNumHeteroatoms(mol))
    heteroatom_fraction = heteroatom_count / heavy_atom_count if heavy_atom_count > 0 else 0.0
    ...
    features.update(
        {
            ...
            "aromatic_ring_density": _as_float(aromatic_ring_density),
            ...
            "heteroatom_fraction": _as_float(heteroatom_fraction),
