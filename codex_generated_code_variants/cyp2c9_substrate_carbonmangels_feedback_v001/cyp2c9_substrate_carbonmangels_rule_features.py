...
MISSING_BASIC_PKA_SENTINEL = 0.0
MISSING_ACIDIC_PKA_SENTINEL = 14.0
MISSING_HEAVY_DEGREE_SENTINEL = 0.0
...
def _oxidation_site_degree_stats(mol, oxidation_atom_indices: set[int]) -> tuple[float, float]:
    if not oxidation_atom_indices:
        return MISSING_HEAVY_DEGREE_SENTINEL, MISSING_HEAVY_DEGREE_SENTINEL
...
def _min_distance_between_atom_sets(
    ...
    if not source_sets or not target_indices:
        return float(mol.GetNumBonds() + 1)
...
def _compute_pka_summary(mol, logp: float) -> dict[str, float]:
    result = {
        "most_basic_pka": MISSING_BASIC_PKA_SENTINEL,
        "most_acidic_pka": MISSING_ACIDIC_PKA_SENTINEL,
        "num_basic_sites": 0.0,
    ...
    most_basic_pka = max(base_sites.values()) if base_sites else MISSING_BASIC_PKA_SENTINEL
    most_acidic_pka = min(acid_sites.values()) if acid_sites else MISSING_ACIDIC_PKA_SENTINEL
