...
def _compute_pka_summary(mol) -> dict[str, float]:
    ...
    result = {
        # Keep pKa-derived columns finite so preprocessing does not drop them.
        "most_basic_pka": 0.0,
        "most_acidic_pka": 0.0,
        "num_basic_sites": 0.0,
    ...
    most_acidic_pka = min(acid_sites.values()) if acid_sites else 0.0
...
def featurize_smiles(smiles: str) -> dict[str, float]:
    ...
    basic_aryl_halide_risk_flag = _flag(aryl_halide_count > 0 and most_basic_pka >= 7.0)
