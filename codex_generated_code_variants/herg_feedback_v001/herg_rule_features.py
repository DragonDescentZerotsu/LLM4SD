...
DEFAULT_MOST_BASIC_PKA = 0.0
DEFAULT_MISSING_BASIC_CENTER_DISTANCE = 7.0
DEFAULT_MISSING_BASIC_TO_AROMATIC_DISTANCE = 5.0
...
def _min_pairwise_distance(
    ...
    *,
    ...
    return missing_value if math.isinf(min_distance) else float(min_distance)
...
def _compute_pka_summary(mol, logp: float) -> dict[str, float]:
    result = {
        "most_basic_pka": DEFAULT_MOST_BASIC_PKA,
        "num_basic_sites": 0.0,
    ...
    most_basic_pka = max(base_sites.values()) if base_sites else DEFAULT_MOST_BASIC_PKA
...
def featurize_smiles(smiles: str) -> dict[str, float]:
    ...
    min_basic_center_distance = _min_pairwise_distance(
        mol,
        basic_center_atom_indices,
        missing_value=DEFAULT_MISSING_BASIC_CENTER_DISTANCE,
    )
    ...
    min_basic_to_aromatic_distance = _min_distance_between_sets(
        ...
        basic_center_atom_indices,
        aromatic_atom_indices,
        missing_value=DEFAULT_MISSING_BASIC_TO_AROMATIC_DISTANCE,
    )
    ...
    cation_pi_proxy_present = bool(
        basic_center_atom_indices
        and aromatic_atom_indices
        and min_basic_to_aromatic_distance <= 4.0
    )

    short_basic_center_distance_present = bool(
        len(basic_center_atom_indices) > 1
        and min_basic_center_distance <= 6.0
    )

    long_basic_to_aromatic_distance_present = bool(
        basic_center_atom_indices
        and aromatic_atom_indices
        and min_basic_to_aromatic_distance > 5.0
    )
